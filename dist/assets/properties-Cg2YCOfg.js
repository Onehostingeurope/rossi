import{s as f}from"./supabase-CsOZGM-w.js";/* empty css              */let o=[],s=[];async function g(){const e=document.getElementById("loading");try{const{data:t,error:r}=await f.from("properties").select("*").order("id",{ascending:!1});if(r)throw r;o=t||[],s=[...o],e&&(e.style.display="none"),p()}catch(t){console.error("Error fetching properties:",t.message),e&&(e.innerHTML=`<p class="error">Erreur de chargement: ${t.message}</p>`)}}const l=document.getElementById("listings-grid"),n=document.getElementById("results-count"),i=document.getElementById("no-results");function m(e){l&&(l.innerHTML="",e.length===0?(i&&(i.style.display="block"),n&&(n.textContent="0 biens trouvés")):(i&&(i.style.display="none"),n&&(n.textContent=`${e.length} biens trouvés`)),e.forEach((t,r)=>{const c=y(t,r);l.appendChild(c)}))}function y(e,t){const r=document.createElement("div");return r.className="prop-card scroll-reveal",r.style.transitionDelay=`${t%3*.1}s`,r.innerHTML=`
  ...
  `,r}const u=document.querySelectorAll("#filter-type .pill"),a=document.getElementById("sort-select");let d="all";function p(){const e=(a==null?void 0:a.value)||"recent";s=o.filter(t=>d==="all"?!0:t.type.toLowerCase().includes(d.toLowerCase())),e==="price-asc"?s.sort((t,r)=>t.price-r.price):e==="price-desc"?s.sort((t,r)=>r.price-t.price):e==="surface-desc"?s.sort((t,r)=>r.surface-t.surface):s.sort((t,r)=>(r.id||0)-(t.id||0)),m(s)}u.forEach(e=>{e.addEventListener("click",t=>{const r=t.target;u.forEach(c=>c.classList.remove("pill-active")),r.classList.add("pill-active"),d=r.getAttribute("data-value")||"all",p()})});a==null||a.addEventListener("change",p);document.addEventListener("DOMContentLoaded",g);
